...
## useState 
  ...
  - 在useState的初值中使用函数会导致每次渲染函数都会执行，如果函数中包含副作用会导致意想不到的结果(比如初值会不按想的方向发展，比如在useState里获取当前时间，会导致本来想要保存的值不停的改变)，当然，执行之后并不会把返回值更新给state了。
...
function FancyInput(props, ref) {
  const inputRef = useRef();
    // 类似一个代理，父组件的ref的操作在这里被映射为inputRef的操作
